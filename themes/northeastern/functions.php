...
// automagically load custom post type files that are in the directory (meaning they are active)
$dir = new DirectoryIterator(get_template_directory()."/functions/customposts/");
foreach ($dir as $file) {
  if(strpos($file->getFilename(),'.') > 0){ // skip any file that begins with a dot
    require_once(get_template_directory()."/functions/customposts/".$file->getFilename());
  }
}
